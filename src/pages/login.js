...
const handleForgotPasswordSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        const adminEmail = data.admin[0].email; // Assuming there's only one admin
        if (email === adminEmail) {
          // Step 2: Generate and send password reset link to the admin's email
          const resetResponse = await fetch('https://toothpix-backend.onrender.com/api/app/admin/request-reset-password', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
          ...
          if (resetResponse.ok) {
            ...
            setMessage2(resetData.message2 || 'Error requesting password reset');
          }
        } else {
          setMessage2('Email does not match the admin account.');
        }
      } else {
      ...
      {showModal && (
        <div className="modal-overlay">
          <div className="modal-content">
            <h2 className='big-text-with-border'>Reset Password</h2>
